...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    check_str = str(pow(3, player_score + opponent_score))
    return check_str[0] == check_str[-1]
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, max_score, max_rolls = 1, 0, 1
    while num_rolls <= 10:
        avg_score = make_averaged(roll_dice, num_samples)
        poss_max_score = avg_score(num_rolls, dice)
        if poss_max_score > max_score:
            max_score, max_rolls = poss_max_score, num_rolls
        num_rolls += 1
    return max_rolls
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    bacon = bacon_strategy(new_score, opponent_score, margin, num_rolls)
    if swap and opponent_score > new_score:
        return 0
    if swap and not bacon:
        return num_rolls
    return bacon
    # END PROBLEM 11
